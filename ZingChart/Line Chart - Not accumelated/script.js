...
// WWWWWWWWWWWORRRRRRRRRRKKKKKKKKKING
	...
	// 	dbt[i]=Number(result.data[i].netAmt);
	// }
...
//JUSSSSSSSSSSSSTTTTTT fine
	...
	for (i in result.data){
		...
		if (dbtAccounts[accName]){
			dbtAccounts[accName].push(Number(result.data[i].netAmt) - lastNetAmt[accName]);	
		}
		else{
			dbtAccounts[accName]=[Number(result.data[i].netAmt)];	
		}	
		lastNetAmt[accName] = Number(result.data[i].netAmt);
	}
